...
def calCrossReads(bamfile):
    dict_Interact_Big = {}
    for read in bamfile:
        if read.is_unmapped == False and read.mate_is_unmapped == False and read.reference_id != read.next_reference_id and read.flag < 2048: 
            read_length_list = []
...
def clusterBasedOnDensity(dict_xy,key):
    ...
    db = DBSCAN(eps = insert_size*2, min_samples = minSample).fit(XY)
    labels = db.labels_
    cluster_label_dict = {}
    lab = labels.tolist()
...
def calculateClusterDictCross(cluster_label_dict,dict_xy,ref_dict_Interact_Big,cross_name,key):
    ...
    for key1 in cluster_label_dict:
        ...
        for i in range(0,len(cluster_label_dict.get(key1))):
            ...
            ls.append(tmp_ls)
        tmp_dict = {key1:ls}
        cluster_dict_cross.update(tmp_dict)
    return cluster_dict_cross    


def calculateClusterCrossRegion(cluster_dict_cross):
    cluster_cross_region    =   []
    for key1 in cluster_dict_cross:
        starts1 =   []
        starts2 =   []
        for i in range(0,len(cluster_dict_cross.get(key1))):
            starts1.append(cluster_dict_cross.get(key1)[i][0].reference_start)
            starts2.append(cluster_dict_cross.get(key1)[i][1].reference_start)
...
def uniqueCrossCluster(sim_bw_cluster_cross, ref_A):
    unique_sim_cross_index  =   []
    unique_sim_cross_region =   []
    for i in range(0,len(sim_bw_cluster_cross)):
        al = parasail.sw_stats_striped_64(sim_bw_cluster_cross[i][0], sim_bw_cluster_cross[i][1], 2, -2, parasail.pam100)
        query_length = len(sim_bw_cluster_cross[i][0])
        if query_length > len(sim_bw_cluster_cross[i][1]):
            query_length = len(sim_bw_cluster_cross[i][1])
        if al.matches / query_length < 0.8:
...
def calculateCandidateSolutionDict(x_pos,w,h,ii,cluster_dict_cross,reverse):
    ...
    for k in range(-1, reverse, -1):
        ...
        for t in range(0,len(tmp)):
            ...
            dict_tmp={index:cluster_dict_cross.get(ii)[index][1].reference_start}
        ...
        for t in range(0,len(y_pos_tmp_list)):
            ...
            for tt in range(t,len(y_pos_tmp_list)):
                ...
                candidate_solution_tmp.append(cluster_dict_cross.get(ii)[index])
...
def calCandidateSolution(w,h,unique_sim_cross_index,cluster_cross_region,cluster_dict_cross):
    ...
    for i in range(0,len(unique_sim_cross_index)):
        ...
        for j in range(0,num):
            dict_tmp = {j:cluster_dict_cross.get(ii)[j][0].reference_start}
...
def worker(count, preClusterData, ref_name_list_per, file_name, ref_dict_Interact_Big, ref_genome):
    htgCluster = {}
    scanned_pair_ref = {}
    for key in ref_name_list_per:
        ref_name = key
        multiple_interact_cluster = {}
        for sub_key in preClusterData.get(key):
            ...
            for sub_sub_key in dict_xy:
                ...
                ref_A = calculateRefA(cluster_label_dict,dict_xy,sub_sub_key)
                cluster_dict_cross = calculateClusterDictCross(cluster_label_dict,dict_xy,ref_dict_Interact_Big,cross_name,sub_sub_key)
                cluster_cross_region = calculateClusterCrossRegion(cluster_dict_cross)
                sim_bw_cluster_cross = calculateSim_bw_cluster_cross(cluster_cross_region,ref_name,cross_name, ref_genome)
                unique_sim_cross_index, unique_sim_cross_region = uniqueCrossCluster(sim_bw_cluster_cross, ref_A)
                w = insert_size
                h = insert_size
                candidate_solution = calCandidateSolution(w, h, unique_sim_cross_index, cluster_cross_region, cluster_dict_cross)
                improved_cross_cluster = calImprovedCrossCluster(candidate_solution)
                kindCluster = {sub_sub_key : improved_cross_cluster}
                dict_cluster.update(kindCluster)
                cluster_dict_cross = {}
                sim_bw_cluster_cross = []
                cluster_cross_region = []
                unique_sim_cross_index = []
                ...
                cluster_label_dict = {}
                kindCluster = {}
            dict_tmp = {cross_name : dict_cluster}
            multiple_interact_cluster.update(dict_tmp)
            dict_cluster = {}
            del dict_cluster
        tmp = {ref_name : multiple_interact_cluster}
        htgCluster.update(tmp)
        multiple_interact_cluster = {}
        del multiple_interact_cluster
    print('candidate breakpoints are found.')
    scanned_pair_ref = {}
    del scanned_pair_ref
    f = open(file_name,  "a")
    for key in htgCluster:
        for sub_key in htgCluster.get(key):
            for sub_sub_key in htgCluster.get(key).get(sub_key):
                if sub_sub_key  ==  'read1pos_pos' or sub_sub_key  ==  'read2neg_neg':
                    i   =   0
                    while i <   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)):
                        pos1_left = min(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0])
                        pos1_right = max(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0])
                        pos2_left = min(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1])
                        pos2_right = max(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1])
                        pos1    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0][-1]
                        pos2    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1][0]
                        num_sup =   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0])
                        flag    =   0
                        for j in range(i+1,   len(htgCluster.get(key).get(sub_key).get(sub_sub_key))):
                            pos1_tmp    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0][-1]
                            pos2_tmp    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][1][0]
                            num_sup_tmp =   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
                            if pos1_tmp == pos1 and pos2_tmp == pos2:
                                tmp_pos1_left = min(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
                                tmp_pos1_right = max(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
                                tmp_pos2_left = min(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][1])
                                tmp_pos2_right = max(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][1])
                                if tmp_pos1_left < pos1_left:
                                    pos1_left = tmp_pos1_left
                                if tmp_pos1_right > pos1_right:
                                    pos1_right = tmp_pos1_right
                                if tmp_pos2_left < pos2_left:
                                    pos2_left = tmp_pos2_left
                                if tmp_pos2_right > pos2_right:
                                    pos2_right = tmp_pos2_right
                                num_sup = num_sup + num_sup_tmp
                            else:
                                i = j
                                flag = 1
                                break
                        if flag ==  0:
                            i   =   len(htgCluster.get(key).get(sub_key).get(sub_sub_key))
                ...
                if sub_sub_key  ==  'read1pos_neg' or sub_sub_key  ==  'read2neg_pos':
                    ...
                    while i <   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)):
                        ...
                        pos2_right = max(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1])
                        pos1    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0][-1]
                        pos2    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1][-1]
                        num_sup =   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0])
                        flag    =   0
                        for j in range(i+1,   len(htgCluster.get(key).get(sub_key).get(sub_sub_key))):
                            pos1_tmp    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0][-1]
                            pos2_tmp    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][1][-1]
                            num_sup_tmp =   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
                            if pos1_tmp == pos1 and pos2_tmp == pos2:
                                tmp_pos1_left = min(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
                                tmp_pos1_right = max(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
                ...
                if sub_sub_key  ==  'read1neg_pos' or sub_sub_key  ==  'read2pos_neg':
                    ...
                    while i <   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)):
                        ...
                        pos2_left = min(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1])
                        pos2_right = max(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1])
                        pos1    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0][0]
                        pos2    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][1][0]
                        num_sup =   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)[i][0])
                        flag    =   0
                        for j in range(i+1,   len(htgCluster.get(key).get(sub_key).get(sub_sub_key))):
                            pos1_tmp    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0][0]
                            pos2_tmp    =   htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][1][0]
                            num_sup_tmp =   len(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
                            if pos1_tmp == pos1 and pos2_tmp == pos2:
                                tmp_pos1_left = min(htgCluster.get(key).get(sub_key).get(sub_sub_key)[j][0])
...
def main():
    ...
    ref_genome = pysam.Fastafile(ref_genome_dir)
    ...
    ref_name_list = list(preClusterData.keys())
    num_ref_per = int(len(ref_name_list) / split_num) + 1
    procs = []
    for i in range(0, split_num):
        if i != split_num - 1:
            ref_name_list_per = ref_name_list[i*num_ref_per : (i+1)*num_ref_per]
        else:
            ref_name_list_per = ref_name_list[(split_num-1)*num_ref_per:]
        p = multiprocessing.Process(target = worker, args = (i, preClusterData, ref_name_list_per, output_filename, ref_dict_Interact_Big, ref_genome))
        procs.append(p)
        p.start()
        print(p.pid)
    for proc in procs:
