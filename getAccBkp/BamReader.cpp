...
bool _Bam::SetRegion(const GenomicRegion& gp) {
	...
	if (gp.chr >= m_hdr.NumSequences()) {
		std::cerr << "Chr ID is bigger than n_targets=" << m_hdr.NumSequences() << std::endl;
		return false;
	}
	...
	if (!hts_itr) {
		std::cerr << "Error: Failed to set region." << std::endl;
		return false;
	}
...
BamReader::BamReader() {}
  ...
  bool _Bam::open_BAM_for_reading(ThreadPool t) {
