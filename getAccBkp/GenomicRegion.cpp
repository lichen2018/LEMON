...
int GenomicRegion::GetOverlap(const GenomicRegion& gr) const {
  ...
  std::string GenomicRegion::PointString() const {
    ...
    out << chrToString(chr) << ":" << AddCommas<int>(pos1) << "(" << strand << ")";
    return out.str();
  }
...
bool GenomicRegion::operator>=(const GenomicRegion &b) const {
  ...
  std::string GenomicRegion::ToString() const {
    return chrToString(chr) + ":" + AddCommas<int>(pos1) + "-" + AddCommas<int>(pos2) + "(" +
      strand + ")"; 
  }

std::ostream& operator<<(std::ostream& out, const GenomicRegion& gr) {
  out << gr.chrToString(gr.chr) << ":" << AddCommas<int>(gr.pos1) << "-" << AddCommas<int>(gr.pos2) << "(" << 
    gr.strand << ")"; 
  return out;
}
...
int32_t GenomicRegion::DistanceBetweenEnds(const GenomicRegion &gr) const {
  ...
  GenomicRegion::GenomicRegion(const std::string& tchr, const std::string& tpos1, const std::string& tpos2, const BamHeader& hdr)
  {
    strand = '*';
...
#else
	chr = std::atoi(scrubString(tchr, "chr").c_str());
#endif
      return;
    } else {
