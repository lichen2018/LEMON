...
Aligner::Aligner(void)
    ...
    , match_score_(2)
    , mismatch_penalty_(2)
    , gap_opening_penalty_(0)
    , gap_extending_penalty_(0)
                    ...
                    Alignment* alignment, const int32_t maskLen) const
{
  ...
  alignment->matches = CalculateNumberMatch(&*alignment, translated_reference_, translated_query, query_len);
                    ...
                    const Filter& filter, Alignment* alignment, const int32_t maskLen) const
{
  ...
  alignment->Clear();
  ConvertAlignment(*s_al, query_len, alignment);
  alignment->matches = CalculateNumberMatch(&*alignment, translated_ref, translated_query, query_len);
